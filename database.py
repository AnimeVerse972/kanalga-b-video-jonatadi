...
async def get_kino_by_code(code):
    async with db_pool.acquire() as conn:
        row = await conn.fetchrow("SELECT * FROM kino WHERE code = $1", code)
        return row

async def get_all_codes():
    async with db_pool.acquire() as conn:
        rows = await conn.fetch("SELECT * FROM kino ORDER BY message_id")
        return rows

async def delete_kino_code(code):
    async with db_pool.acquire() as conn:
        result = await conn.execute("DELETE FROM kino WHERE code = $1", code)
        return result.endswith("1")  # agar 1 ta o'chirildi degani
